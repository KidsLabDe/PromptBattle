<script lang="ts">
	import { generatedImage, generationError, generationStep, generationTotal, uiState } from '$lib/stores/gameStore';

	let isGenerating = $derived($uiState === 'generating');
	let hasImage = $derived(!!$generatedImage);
	let hasError = $derived(!!$generationError);
</script>
	...
	<div class="aspect-square w-full overflow-hidden rounded-xl border-2 border-neon-green/30 bg-bg-card">
		...
		{:else if hasError}
			<div class="flex h-full flex-col items-center justify-center gap-3 p-4">
				...
				<p class="text-center text-sm text-gray-400 break-words max-w-full">{$generationError}</p>
			</div>
		{:else if hasImage}
